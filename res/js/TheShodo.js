...
(function($TS) {
    ...
    $TS.UA.isHTML5Supported = function(elementName, attrName) {
        return (attrName in _d.createElement(elementName));
    }
    ...
    $TS.UA.isCSS3CalcSupported = (function() {
        var e = document.createElement('div');
        e.style.cssText = 'width: calc(10px + 10px)';
        return e.style.cssText.match('calc');
    })();
    $TS.UA.isSiteMode = (function() {
        try {
            return (window.external && window.external.msIsSiteMode());
        } catch (e) {}
        return false;
    })();
    $TS.OS = {};
    ...
    $TS.ENV.load = function() {
        ...
        if (storage && storage["TheShodo.volume"] && storage["TheShodo.muted"]) {
            this.setVolume(
                parseFloat(storage["TheShodo.volume"], 10), storage["TheShodo.muted"] == 'true' ? true : false, 'noStorage'
            );
        }
    };
